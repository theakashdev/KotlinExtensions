...
class BubbleDrawable : Drawable() {
    ...
    override fun draw(canvas: Canvas) {
        ...
        if (triangleLoc == locTop) {
            mPath.lineTo(
                bounds.left + bounds.width() * triangleBias - triangleWidth / 2,
        ...
        if (triangleLoc == locRight) {
            mPath.lineTo(
                rightTopRectF.right,
                bounds.top + bounds.height() * triangleBias - triangleHeight / 2
            )
            mPath.lineTo(
                bounds.right.toFloat() - strokeWidth,
                bounds.top + bounds.height() * triangleBias
            )
            mPath.lineTo(
        ...
        if (triangleLoc == locBottom) {
            mPath.lineTo(
                bounds.right - bounds.width() * triangleBias + triangleWidth / 2,
        ...
        if (triangleLoc == locLeft) {
            ...
            mPath.lineTo(
                bounds.left.toFloat(),
                leftBottomRectF.bottom - bounds.height() * triangleBias
            )
            mPath.lineTo(
                leftTopRectF.left,
                leftBottomRectF.bottom - bounds.height() * triangleBias - triangleHeight / 2
    ...
    private fun measureLeftTopCorner() {
        bounds.apply {
            leftTopRectF.left =
                if (triangleLoc == locLeft) left.toFloat() + triangleWidth else left.toFloat()
            leftTopRectF.top =
                if (triangleLoc == locTop) top.toFloat() + triangleHeight else top.toFloat() + strokeWidth
    ...
    private fun measureRightTopCorner() {
        bounds.apply {
            rightTopRectF.right =
                if (triangleLoc == locRight) right.toFloat() - triangleWidth else right.toFloat()
            rightTopRectF.top =
                if (triangleLoc == locTop) top.toFloat() + triangleHeight else top.toFloat() + strokeWidth
    ...
    private fun measureRightBottomCorner() {
        bounds.apply {
            rightBottomRectF.right =
                if (triangleLoc == TriangleLocation.locRight) right.toFloat() - triangleWidth else right.toFloat()
            rightBottomRectF.bottom =
                if (triangleLoc == TriangleLocation.locBottom) bottom.toFloat() - triangleHeight else bottom.toFloat()
    ...
    private fun measureLeftBottomCorner() {
        bounds.apply {
            leftBottomRectF.left =
                if (triangleLoc == TriangleLocation.locLeft) left.toFloat() + triangleWidth else left.toFloat()
            leftBottomRectF.bottom =
                if (triangleLoc == TriangleLocation.locBottom) bottom.toFloat() - triangleHeight else bottom.toFloat()
